...
现有一段祖传代码，为了跨平台兼容性，需要从WCF改成Grpc。

这段代码来源于鄙人公司首席架构师先生若干年的积累，虽然代码中异味比较多【例如没有分层、面向过程式、例如变量命名、例如代码重复率高、例如一个类中代码太多（嗯。。五千行，共有操作合约方法300个）等，让鄙人费劲头脑，但原因可以理解的，毕竟"~~企业飞速发展过程中的每个阶段都必须经过不同的考验，而代码这种文档化的东西往往必须适应市场变化的需求~~"(甲方爸爸改需求，谁都挡不住）】，所以作为吃屎侠的我们，必须硬着头皮把这一盆热翔吃下去，而且吃完还得说“哇，Fresh Meat"。

在忽略了这段代码的整洁性之后，我们得根据目前的实际需求来迁移我们的代码了。而在迁移之前，我们可以根据目前掌握的技术信息提炼加工，抽象化出一些可供我们参考使用的实践模式。
